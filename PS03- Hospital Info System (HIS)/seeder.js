...
const importData = async () => {
    try {
        ...
        // WARDS AND BEDS
        const wardsData = [
            { name: 'General Ward', prefix: 'GW', type: 'general', ratePerDay: 500, floor: 'Ground Floor', description: 'Standard care ward for general patients' },
            { name: 'ICU', prefix: 'ICU', type: 'icu', ratePerDay: 5000, floor: '1st Floor', description: 'Intensive Care Unit for critical patients' },
            { name: 'Private Rooms', prefix: 'PR', type: 'private', ratePerDay: 3000, floor: '2nd Floor', description: 'Premium private rooms with amenities' },
            { name: 'Semi-Private', prefix: 'SP', type: 'semi-private', ratePerDay: 1500, floor: '2nd Floor', description: 'Semi-private rooms with 2 beds' },
            { name: 'Pediatric Ward', prefix: 'PED', type: 'pediatric', ratePerDay: 800, floor: '3rd Floor', description: 'Specialized ward for children' },
            { name: 'Maternity Ward', prefix: 'MAT', type: 'maternity', ratePerDay: 1200, floor: '3rd Floor', description: 'Maternity and post-natal care' },
            { name: 'Emergency Ward', prefix: 'ER', type: 'emergency', ratePerDay: 2000, floor: 'Ground Floor', description: 'Emergency department beds' },
            { name: 'Surgical Ward', prefix: 'SURG', type: 'surgical', ratePerDay: 1800, floor: '1st Floor', description: 'Post-operative recovery ward' },
            { name: 'Cardiac Care', prefix: 'CCU', type: 'cardiac', ratePerDay: 4000, floor: '1st Floor', description: 'Cardiac Care Unit' },
            { name: 'Orthopedic Ward', prefix: 'ORTH', type: 'orthopedic', ratePerDay: 1200, floor: '2nd Floor', description: 'Orthopedic and trauma patients' }
        ];

        const wards = [];
        for (let i = 0; i < wardsData.length; i++) {
            const wardData = wardsData[i];
            const ward = await Ward.create({
                wardId: `WARD-${String(i + 1).padStart(3, '0')}`,
                ...wardData,
                totalBeds: 0
            });
            wards.push(ward);
        ...
        // Sample patient names for occupied beds
        const bedPatientNames = [
            'Ramesh Kumar', 'Sunita Devi', 'Anil Sharma', 'Kavita Patel', 'Vikram Singh',
            'Neha Gupta', 'Rajesh Verma', 'Priya Mehta', 'Suresh Yadav', 'Anita Rani',
            'Manoj Tiwari', 'Geeta Kumari', 'Deepak Joshi', 'Rekha Agarwal', 'Sanjay Mishra',
            'Meena Devi', 'Ashok Kumar', 'Pooja Sharma', 'Ravi Shankar', 'Kiran Bala'
        ];
        const bedDiagnoses = [
            'Pneumonia', 'Cardiac Monitoring', 'Post-Surgery Recovery', 'Fracture Treatment',
            'Diabetes Management', 'Fever Investigation', 'Appendectomy Recovery', 'Hypertension',
            'Respiratory Infection', 'Post-Delivery Care', 'Orthopedic Surgery', 'Dengue Treatment',
            'Kidney Stones', 'Gastric Issues', 'Chest Pain Investigation', 'Anemia Treatment'
        ];
        const doctors = ['Dr. Sharma', 'Dr. Patel', 'Dr. Singh', 'Dr. Kumar', 'Dr. Gupta', 'Dr. Mehta'];

        // Beds per ward configuration
        const bedsConfig = [
            { wardIndex: 0, numBeds: 20 },  // General Ward - 20 beds
            { wardIndex: 1, numBeds: 10 },  // ICU - 10 beds
            { wardIndex: 2, numBeds: 15 },  // Private Rooms - 15 beds
            { wardIndex: 3, numBeds: 12 },  // Semi-Private - 12 beds
            { wardIndex: 4, numBeds: 10 },  // Pediatric - 10 beds
            { wardIndex: 5, numBeds: 8 },   // Maternity - 8 beds
            { wardIndex: 6, numBeds: 12 },  // Emergency - 12 beds
            { wardIndex: 7, numBeds: 10 },  // Surgical - 10 beds
            { wardIndex: 8, numBeds: 8 },   // Cardiac Care - 8 beds
            { wardIndex: 9, numBeds: 10 }   // Orthopedic - 10 beds
        ];

        let patientIndex = 0;
        for (const config of bedsConfig) {
            const ward = wards[config.wardIndex];
            const beds = [];
            
            for (let i = 1; i <= config.numBeds; i++) {
                const bedNumber = `${ward.prefix}-${String(i).padStart(3, '0')}`;
                
                // Determine bed status - mix of statuses for realistic demo
                ...
                if (rand < 0.45) {
                    ...
                    status = 'occupied';
                    patient = {
                        name: bedPatientNames[patientIndex % bedPatientNames.length],
                        diagnosis: bedDiagnoses[patientIndex % bedDiagnoses.length],
                        admitDate: new Date(Date.now() - Math.floor(Math.random() * 7) * 24 * 60 * 60 * 1000),
                        doctor: doctors[Math.floor(Math.random() * doctors.length)]
                    };
                    patientIndex++;
                } else if (rand < 0.55) {
                    // 10% critical (only for ICU, CCU, Emergency)
                    if ([1, 6, 8].includes(config.wardIndex)) {
                        status = 'critical';
                        patient = {
                            name: bedPatientNames[patientIndex % bedPatientNames.length],
                            diagnosis: 'Critical - ' + bedDiagnoses[patientIndex % bedDiagnoses.length],
                            admitDate: new Date(Date.now() - Math.floor(Math.random() * 3) * 24 * 60 * 60 * 1000),
                            doctor: doctors[Math.floor(Math.random() * doctors.length)]
                        };
                        patientIndex++;
                    }
                ...
                // Rest (35%) remain available

                const bed = await Bed.create({
                    bedId: `${ward.wardId}-${bedNumber}`,
                    number: bedNumber,
                    ward: ward._id,
                    status,
                    patient,
            ...
            // Update ward with total beds count
            await Ward.findByIdAndUpdate(ward._id, { totalBeds: config.numBeds });
        }
